...
export const { use: useLocal, provider: LocalProvider } = createSimpleContext({
  ...
  init: () => {
    ...
    // ============================================================================
    // Multi-Model Support (Agents R)
    // ============================================================================
    function createMultiModel() {
      const MAX_MODELS = 3

      const [multiModelStore, setMultiModelStore] = createStore<{
        active: {
          id: string
          name: string
          providerID: string
      ...
      const filePath = path.join(paths.state, "multi-model.json")
      const state = {
        pending: false,
      }

      function save() {
        if (!multiModelStore.ready) {
          state.pending = true
          return
        }
        state.pending = false
        void writeJsonAtomic(filePath, {
          active: multiModelStore.active,
        })
      }
      ...
      readJson<unknown>(filePath)
        .then((x) => {
          ...
          if (Array.isArray(value.active)) {
            setMultiModelStore("active", value.active)
          }
        })
        .catch(() => {})
        .finally(() => {
          setMultiModelStore("ready", true)
          if (state.pending) save()
        })

      return {
        get ready() {
          return multiModelStore.ready
        },
        list() {
          return multiModelStore.active
        },
        count() {
          return multiModelStore.active.length
        },
        add(name: string, providerID: string, modelID: string) {
          if (multiModelStore.active.length >= MAX_MODELS) {
            toast.show({
              variant: "warning",
              message: `Maximum ${MAX_MODELS} models allowed`,
              duration: 3000,
            })
            return false
          }

          if (multiModelStore.active.some((m) => m.providerID === providerID && m.modelID === modelID)) {
            toast.show({
              variant: "warning",
              message: "Model already added",
              duration: 3000,
            })
            return false
          }
          ...
          if (!isModelValid({ providerID, modelID })) {
            toast.show({
              variant: "warning",
              message: `Invalid model: ${providerID}/${modelID}`,
              duration: 3000,
            })
            return false
          }
          ...
          batch(() => {
            setMultiModelStore("active", (prev) => [
              ...prev,
              {
        ...
        remove(id: string) {
          batch(() => {
            setMultiModelStore("active", (prev) => prev.filter((m) => m.id !== id))
            save()
          })
        },
        ...
        update(id: string, providerID: string, modelID: string) {
          if (!isModelValid({ providerID, modelID })) {
            toast.show({
              variant: "warning",
              message: `Invalid model: ${providerID}/${modelID}`,
          ...
          batch(() => {
            setMultiModelStore("active", (prev) =>
              prev.map((m) => (m.id === id ? { ...m, providerID, modelID } : m)),
            )
            save()
          })
          return true
        ...
        clear() {
          batch(() => {
            setMultiModelStore("active", [])
            save()
          })
        },
        hasId(id: string) {
          return multiModelStore.active.some((m) => m.id === id)
        },
        getById(id: string) {
          return multiModelStore.active.find((m) => m.id === id)
        },
      }
    }

    const multiModel = createMultiModel()
